...
int main(int argc,char *argv[])
{
	...
	double sample_duration = 10; // PM2.5量測時間
	...
	double direction = rand() % 360; // 搜尋方位角D
	double windDis = 5; // 初始行為移動距離
	int far_c = argc>1?atoi(argv[1]):20; // PM2.5背景濃度C_min
	int close_c = argc>2?atoi(argv[2]):200;// PM2.5排放源濃度C_max
	int area = argc>3?atoi(argv[3]):3600;// 搜尋面積A
	double r_m = argc>4?atoi(argv[4]):10;// 行進距離比(倒數)
	double r_s = argc>5?atoi(argv[5]):3;// 取樣距離比(倒數)
	double time_limit = argc>6?atoi(argv[6]):1200;// 飛行時間上限

    // 根據公式比計算最大/最小 行進/取樣距離
	...
	*/
    ...
    for(int i=0;i<stat.sample.size();i++){    
        cout<<argv[1]<<","<<argv[2]<<","<<"dynamic"<<","<<argv[4]<<","<<argv[5]<<","<<argv[3]<<","<<argv[6];
